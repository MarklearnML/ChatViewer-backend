...
import com.chatviewer.blog.dto.ProblemDto;
...
public class ProblemController {
     ...
     */
    @PostMapping("/update")
    public Result<Object> updateProblem(@RequestBody Map<String, Object> map) {
        Problem problem = BeanUtil.mapToBean(map, Problem.class, false, CopyOptions.create());
        // 从分类列表取得分类Id
        List<String> categoryList = (List<String>) map.get("categoryList");
        problem.setCategoryId(Long.parseLong(categoryList.get(categoryList.size() - 1)));
    ...
    public Result<Object> deleteProblem(@RequestParam Long problemId) {
        Long userId = ContextHolderUtil.getUserId();
        boolean remove = problemMapper.deleteProblemByIdAndUserId(problemId, userId);
        if(remove!=true) {
            return Result.fail();
        }
        return Result.success();
    ...
    public Result<Object> getById(Long problemId) {
        ...
        if(problem.getCreateUser().equals(userId)) {
            problemDto.setIsUser(true);
        }
